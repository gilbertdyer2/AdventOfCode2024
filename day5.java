...
public class day5 {
    public static void main(String[] args) {
        invalidUpdates = new ArrayList<Integer>();
        initInput();
        initRulesMap();
        initRequiredPrecursorMap();
        System.out.println(middleSum()); // also intializes invalid list
        System.out.println(invalidSum());

    }
    
    public static int middleSum() {
        int sum = 0;
        // Strategy for finding correct updates:
        // 1. Hash all rules, <int, arraylist<int>>. e.g. given 97|13 and 97|61, 97 would be mapped to {13, 61}
        // 2. Iterate through each update:
        //      - Create a hashset and add to it after each iteration
        //      - At each update(i), check if the hashset contains any numbers in the arraylist mapped to update(i)
        //      - If it contains one of the nums, the update is invalid

        for (int i = 0; i < updates.length; i++) {
            HashSet<Integer> seen = new HashSet<>();
            boolean invalidFound = false;
            for (int j = 0; j < updates[i].length; j++) {
                // check arraylist tied to updates[i]
                if (rulesMap.containsKey(updates[i][j])) {
                    ArrayList<Integer> invalidNums = rulesMap.get(updates[i][j]);
                    for (int k = 0; k < invalidNums.size(); k++) {
                        if (seen.contains(invalidNums.get(k))) {
                            System.out.println("invalid found: " + i);
                            invalidUpdates.add(i);
                            invalidFound = true;
                            break;
                        }
                    }
                    // break early to prevent invalidUpdates from accumulating dupes
                    if (invalidFound)
                        break;
                }
                seen.add(updates[i][j]);
            }
            if (!invalidFound) {
                // add middle value
                sum += updates[i][updates[i].length / 2];
                // System.out.println("valid found: " + i);
            }
        }
    ...
    private static int invalidSum() {
        int sum = 0;
        // bruteforce - Once again, build hashset, then use while(true) loop to build a valid array
        //              based on the rules
        for (int i : invalidUpdates) {
            HashSet<Integer> seen = new HashSet<>(); // update as we put down nums
            ArrayList<Integer> numsList = new ArrayList<>();
            for (int j = 0; j < updates[i].length; j++) {
                numsList.add(updates[i][j]);
            }

            ArrayList<Integer> correctedUpdate = new ArrayList<>();
            // Assume there will always be a correct way to order the updates
            while (numsList.size() > 0) {
                // iterate through each element we still have to place
                for (int j = 0; j < numsList.size(); j++) {
                    // if either there are no mapped requirements, or we loop through and all are present,
                    // then append this num to correctedUpdate
                    boolean canPlaceNum = false;
                
                    if (!requireMap.containsKey(numsList.get(j)))
                        canPlaceNum = true;
                    
                    if (requireMap.containsKey(numsList.get(j))) {
                        // System.out.println("placing num @: " + numsList.get(j).toString());
                        ArrayList<Integer> requiredElements = requireMap.get(numsList.get(j));
                        
                        boolean isValid = true;
                        for (int k : requiredElements) {
                            if (!seen.contains(k) && numsList.contains(k)) {
                                isValid = false;
                                break;
                            }
                        }
                    ...
                    // If we can place the number, then append the number to correctedUpdate and add it to seen
                    if (canPlaceNum) {
                        
                        int numToPlace = numsList.get(j);
                        seen.add(numToPlace);
                        correctedUpdate.add(numToPlace);
                        numsList.remove(Integer.valueOf(numToPlace));
                        // Break for simplicity to start at index 0 once again
                        break;
                    }
            ...
            // Now we have the corrected update, so add the middle to our sum
            sum += correctedUpdate.get(correctedUpdate.size() / 2);
        }
    ...
    // Precursor map "reverses" each rule, so that each integer shows which ints need to come before it
    private static void initRequiredPrecursorMap() {
        HashMap<Integer, ArrayList<Integer>> resMap = new HashMap<>();
        for (int i = 0; i < rules.length; i++) {
            if (!resMap.containsKey(rules[i][1])) {
                resMap.put(rules[i][1], new ArrayList<Integer>());
            }

            resMap.get(rules[i][1]).add(rules[i][0]);
        }
        System.out.println(resMap.get(46).toString());
        requireMap = resMap;
    }
    ...
    private static void initRulesMap() {
        HashMap<Integer, ArrayList<Integer>> resMap = new HashMap<>();
        for (int i = 0; i < rules.length; i++) {
            if (!resMap.containsKey(rules[i][0])) {
                resMap.put(rules[i][0], new ArrayList<Integer>());
            }

            resMap.get(rules[i][0]).add(rules[i][1]);
        }
    ...
    private static ArrayList<Integer> invalidUpdates;
    private static int[][] rules;
    private static HashMap<Integer, ArrayList<Integer>> rulesMap;
    private static HashMap<Integer, ArrayList<Integer>> requireMap;
    ...
    private static void initInput() {
        ...
        rules = res;
        // Initialize updates
        String[] linesUpdate = input.split("\n");

        int[][] resUpdate = new int[linesUpdate.length][];
        for (int i = 0; i < linesUpdate.length; i++) {
            String[] splitComma = linesUpdate[i].split(",");
            int[] nums = new int[splitComma.length];
            for(int j = 0; j < splitComma.length; j++) {
                nums[j] = Integer.parseInt(splitComma[j]);
            }
    ...
    private static final String inputRules = """
    83|24
    88|31
    88|42
    34|24
